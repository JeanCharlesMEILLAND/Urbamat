...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const real = await getRealisation(params.slug);
        ...
        <Container>
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap items-center gap-6 mt-4 text-gray-500">
              ...
              <span className="flex items-center gap-1.5">
                <Building2 size={16} />
                {real.nbStations} station{real.nbStations > 1 ? "s" : ""}
              </span>
            </div>
          </div>
        </Container>
            ...
            <div className="bg-neutral-light rounded-lg p-6 border border-gray-200 h-fit">
              ...
              <dl className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="font-medium text-neutral-dark">{real.ville}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-gray-500">Département</dt>
                  <dd className="font-medium text-neutral-dark">{real.departement}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-gray-500">Année</dt>
                  <dd className="font-mono font-medium text-neutral-dark">{real.annee}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-gray-500">Typologie</dt>
                  <dd className="font-medium text-neutral-dark text-right">{real.typologieQuai}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-gray-500">Longueur totale</dt>
                  <dd className="font-mono font-medium text-primary">{real.longueurMl} ml</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-gray-500">Stations</dt>
                  <dd className="font-mono font-medium text-primary">{real.nbStations}</dd>
                </div>
              </dl>
      ...
      <section className="py-8 bg-neutral-light border-t border-gray-200">
        <Container>
          <div className="flex items-center justify-between">
            <Link
              href="/realisations"
              className="inline-flex items-center gap-2 text-sm text-primary font-medium hover:text-primary-600 transition-colors"
            >
              <ArrowLeft size={16} />
